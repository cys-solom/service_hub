...
const C = {
    ...
    text: '#E8E8E8', textSec: '#686868', textMuted: '#404040',
...
export default function CartPage() {
    ...
    const handleSendOrder = async () => {
        ...
        try {
            await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customerName, customerPhone, items: items.map(i => ({ productId: i.productId, productName: i.productName, variant: i.variantTitle, price: i.price, quantity: i.quantity })), totalPrice: finalPrice, notes: customerNotes, couponCode: appliedCoupon?.code || null }) });
        } catch (err) { console.error('Failed to save order', err); }
        const message = buildWhatsAppMessage({ orderCode, customerName, customerPhone, notes: customerNotes, items: items.map(i => ({ productName: i.productName, variant: i.variantTitle, price: i.price, quantity: i.quantity })), totalPrice: finalPrice, currency: currency || 'EGP', locale });
              ...
              <p style={{ color: C.textSec, marginBottom: '2rem' }}>{itemCount} {itemCount === 1 ? t.cart.item : t.cart.items} {t.cart.inCart}</p>
                ...
                {items.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '5rem 0' }}>
                        ...
                        <p style={{ color: C.textSec, marginBottom: '2rem' }}>{t.cart.emptyDesc}</p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
                        {/* === RESPONSIVE: On desktop use CSS @media to make 2-col, here we stack === */}
                            ...
                            <div style={{ position: 'sticky', top: '6rem', alignSelf: 'start' }}>
                                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: '1rem', padding: '1.5rem' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1.5rem', marginBottom: '1.5rem' }}>
                                        <input type="text" placeholder={t.cart.namePlaceholder} value={customerName} onChange={e => setCustomerName(e.target.value)} style={inputStyle} />
                                        <input type="tel" placeholder={t.cart.phonePlaceholder} value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} style={inputStyle} />
                                        <textarea placeholder={t.cart.notesPlaceholder} value={customerNotes} onChange={e => setCustomerNotes(e.target.value)} rows={2} style={{ ...inputStyle, resize: 'none' as const }} />
                                    </div>

                                    <button onClick={handleSendOrder} disabled={!customerName || !customerPhone || sending}
                                        style={{ width: '100%', padding: '1rem', background: 'linear-gradient(135deg, #10b981, #059669)', color: 'white', borderRadius: '1rem', fontWeight: 600, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontSize: '0.9375rem', opacity: (!customerName || !customerPhone || sending) ? 0.5 : 1, boxShadow: '0 4px 15px rgba(16,185,129,0.3)' }}>
